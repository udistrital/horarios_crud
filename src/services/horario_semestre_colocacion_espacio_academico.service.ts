...
import { ColocacionEspacioAcademico } from 'src/models/colocacion_espacio_academico.dtoSchema';
import { HorarioSemestre } from 'src/models/horario_semestre.dtoSchema';
import { HorarioSemestreColocacionEspacioAcademico as MainModel, HorarioSemestreColocacionEspacioAcademicoDto as MainDto } from 'src/models/horario_semestre_colocacion_espacio_academico.dtoSchema';

@Injectable()
export class HorarioSemestreColocacionEspacioAcademicoService {
    constructor(
        ...
        @InjectModel(ColocacionEspacioAcademico.name)
        private readonly colocacionEspacioAcademicoModel: Model<ColocacionEspacioAcademico>
    ) { }
    ...
    private async checkRelated(mainDto: MainDto) {
        ...
        if (mainDto.ColocacionEspacioAcademicoId) {
            const colocacionEspacioAcademicoId = await this.colocacionEspacioAcademicoModel.exists({ _id: mainDto.ColocacionEspacioAcademicoId });
            if (!colocacionEspacioAcademicoId) {
                throw new Error(`ColocacionEspacioAcademicoId: ${mainDto.ColocacionEspacioAcademicoId} doesn't exist`);
            }
        }
    }
    ...
    */
    private populatefields(): any[] {
        return [
            ...
            { path: ColocacionEspacioAcademico.name + 'Id' }
        ]
    }
