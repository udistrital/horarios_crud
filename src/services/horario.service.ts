...
import { EstadoCreacion } from 'src/models/estado_creacion.dtoSchema';
import { Horario as MainModel, HorarioDto as MainDto } from 'src/models/horario.dtoSchema';
...
export class HorarioService {
    constructor(
        ...
        @InjectModel(EstadoCreacion.name)
        private readonly estadoCreacionModel: Model<EstadoCreacion>,
    ) { }
    ...
    private async checkRelated(mainDto: MainDto) {
        if (mainDto.EstadoCreacionId) {
            const estadoCreacionId = await this.estadoCreacionModel.exists({ _id: mainDto.EstadoCreacionId });
            if (!estadoCreacionId) {
                throw new Error(`EstadoCreacionId: ${mainDto.EstadoCreacionId} doesn't exist`);
            }
        }
    }
